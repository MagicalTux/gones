...
$(PROJECT_NAME): $(SOURCES)
	...
	$(GOROOT)/bin/go build -v -gcflags="-N -l" -ldflags=all="-X github.com/KarpelesLab/goupd.PROJECT_NAME=$(PROJECT_NAME) -X github.com/KarpelesLab/goupd.MODE=DEV $(GOLDFLAGS)"
...
dist/$(PROJECT_NAME)_$(GIT_TAG)/build_$(PROJECT_NAME).$(TARGET_ARCH): $(SOURCES)
	@GOOS="$(TARGET_GOOS)" GOARCH="$(TARGET_GOARCH)" $(GOROOT)/bin/go build -a -o "$@" -gcflags="-N -l -trimpath=$(shell pwd)" -ldflags=all="-s -w -X github.com/KarpelesLab/goupd.PROJECT_NAME=$(PROJECT_NAME) -X github.com/KarpelesLab/goupd.MODE=PROD $(GOLDFLAGS)"
endif
